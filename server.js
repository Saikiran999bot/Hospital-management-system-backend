...
const ADMIN_ID   = process.env.ADMIN_EMAIL || "admin@hospital.com";
...
<div style="max-width:520px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08)">
  <div style="background:linear-gradient(135deg,#0B6E6E,#11B5B5);padding:24px 28px;color:#fff">
...
app.post("/api/admin/send-change-password-otp", adminAuth, async (req, res) => {
  try {
    ...
    if (!adminEmail) {
      return res.status(400).json({ error: "Admin email not configured in API Keys" });
    }

    // Generate and save OTP
    const otp = generateOTP();
    saveOTP(adminEmail, otp, "change_password");

    // Send email with OTP
    await sendEmail(
      adminEmail,
      "🔐 Change Password OTP — MediCare+ Admin",
      emailHtml(
        "blue",
        "🔑",
        ...
        `<p>You requested to change your admin password.</p>
         ...
         <h2 style="font-size:32px;letter-spacing:8px;font-weight:800;color:#1D4ED8;margin:16px 0">${otp}</h2>
...
app.post("/api/admin/confirm-change-password", adminAuth, async (req, res) => {
  try {
    ...
    // Verify OTP
    if (!verifyOTP(adminEmail, otp, "change_password")) {
    ...
    // Update in settings (as JSON object)
    const adminData = { email: adminEmail, password_hash: hashedPass };
    await saveSetting("admin_credentials", JSON.stringify(adminData));

    // Also update in DB if users table has admin
    await supabase
      .from("users")
      .update({ password: hashedPass, updated_at: new Date().toISOString() })
      .eq("email", adminEmail);
